...
const ProductList: React.FC = () => {
  ...
  const handleEditProduct = (id: number) => {
    ...
    const newPrice = parseFloat(prompt('Digite o novo preço do produto:') || '0');
    setProducts(products.map(product =>
      product.id === id
        ? { ...product, name: newName || product.name, price: newPrice || product.price }
        : product
    ));
  };
    ...
    <div>
      ...
      {products.map(product => (
        <div key={product.id} style={{ border: '1px solid #ccc', padding: '10px', margin: '10px 0' }}>
          ...
          <button onClick={() => handleViewDetails(product)}>Ver Detalhes</button>
